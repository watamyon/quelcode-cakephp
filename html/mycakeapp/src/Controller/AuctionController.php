...
class AuctionController extends AuctionBaseController
{
	...
	public function add()
	{
		// Biditemインスタンスを用意。つまりはBiditemsのモデルを通じてDBにアクセスして、新しいエンティティ（レコード）のインスタンスを作っている
		$biditem = $this->Biditems->newEntity();
		// POST送信時の処理
		if ($this->request->is('post')) {
			// ここで、送信があった場合に拡張子をifでチェック、当てはまれば、move_uploaded_file？で移動＆保存？それとも保存は下のsave($biditem)で行われるからやらなくていい？
